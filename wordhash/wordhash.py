...
def get_digest(
    filepath: t.Optional[pathlib.Path] = None, string: str = "", length: int = 8
) -> bytes:
    if bool(filepath) == bool(string):
        raise ValueError("Need exactly one of `filepath` or `string` set")
...
def get_wordhash(data: bytes, wordlist: t.List[str], length: int) -> str:
...
def wordhash(data: bytes) -> str:
    words = get_words(DEFAULT_WORDLIST_PATH.expanduser())
...
def main():
    ...
    parser.add_argument(
        "-l",
        "--length",
        help="Length of hash digest to use. Determines number of words in wordhash.",
        type=int,
